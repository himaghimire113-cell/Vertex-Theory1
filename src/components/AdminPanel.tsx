    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      {activeTab === 'appearance' && (
        <form onSubmit={handleSaveAppearance} className="space-y-6 max-w-3xl">
          ...
          {/* CONTENT PREVIEW LOCK & POPUNDER AD SETTINGS */}
          <div className="p-5 rounded-2xl bg-[#12141c] border border-[#232733] space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <h4 className="font-heading font-bold text-sm text-white flex items-center gap-2">
                  <Lock className="w-4 h-4 text-[#ff5533]" />
                  <span>Post Content Preview & Popunder Unlock Gate</span>
                </h4>
                <p className="text-xs text-[#9ca3af]">
                  First-time visitors see an excerpt preview. Clicking the unlock button triggers a popunder ad and unlocks the full article.
                </p>
              </div>
              <label className="flex items-center gap-2 cursor-pointer shrink-0">
                <input
                  type="checkbox"
                  checked={appearanceData.contentLock?.enabled ?? true}
                  onChange={(e) =>
                    setAppearanceData({
                      ...appearanceData,
                      contentLock: {
                  ...
                  className="w-4 h-4 rounded text-[#ff5533] bg-[#0c0d10] border-[#2b303e]"
                />
                <span className="text-xs text-white font-medium">Enable Gate</span>
              </label>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  value={appearanceData.contentLock?.buttonText ?? 'Click here and comeback'}
                  onChange={(e) =>
                    setAppearanceData({
                      ...appearanceData,
                      contentLock: {
                  ...
                  placeholder="Click here and comeback"
                  className="w-full px-3 py-2 rounded-lg bg-[#0c0d10] border border-[#262a37] text-white text-xs"
                />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-mono text-[#9ca3af]">Preview Length (Paragraphs)</label>
                <input
                  type="number"
                  min="1"
                  max="10"
                  value={appearanceData.contentLock?.previewParagraphs ?? 2}
                  onChange={(e) =>
                    setAppearanceData({
                      ...appearanceData,
                      contentLock: {
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                {appearanceData.contentLock?.adUrl && (
                  <button
                    type="button"
                    onClick={() => {
                      const url = appearanceData.contentLock?.adUrl?.trim();
                      if (url) window.open(url, '_blank');
                    }}
                    className="text-[10px] font-mono text-[#ff7755] hover:underline flex items-center gap-1 cursor-pointer"
                  >
              ...
              <input
                type="url"
                value={appearanceData.contentLock?.adUrl || ''}
                onChange={(e) =>
                  setAppearanceData({
                    ...appearanceData,
                    contentLock: {
            ...
            <div className="space-y-1">
              <label className="text-xs font-mono text-[#9ca3af]">Custom Teaser Note (Optional)</label>
              <input
                type="text"
                value={appearanceData.contentLock?.promptText || ''}
                onChange={(e) =>
                  setAppearanceData({
                    ...appearanceData,
                    contentLock: {
                ...
                placeholder="You are currently viewing a preview. Click below and come back to unlock the full essay."
                className="w-full px-3 py-2 rounded-lg bg-[#0c0d10] border border-[#262a37] text-white text-xs"
              />
            </div>
          </div>
